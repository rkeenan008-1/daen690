...
// Get foreign key joins for a table, and return joined data
app.get("/api/table/:name/joins", async (req, res) => {
  const tableName = req.params.name;
  //console.log(`🔍 Looking up foreign keys for table: ${tableName}`);
  ...
  try {
    const fkInfoSql = `
      ...
      WHERE fk.table_name = $1
        AND fk.table_schema = current_schema()
      ORDER BY fk.constraint_name, fk.ordinal_position;
    `;
    
    const fkResult = await pool.query(fkInfoSql, [tableName]);

    //console.log(`🧩 Raw foreign key rows (${fkResult.rowCount}):`);
    ...
    // Optional: Deduplicate (in case multiple matches appear)
    const uniqueFks = Array.from(
      new Map(
        fkResult.rows.map(fk => [`${fk.fk_table}.${fk.fk_column}`, fk])
      ).values()
    );
    //console.log(`✅ Unique foreign keys (${uniqueFks.length}):`);
    //console.table(uniqueFks);

    // --- Build JOIN SQL
    const tableIdent = format("%I", tableName);
    const joinClauses = uniqueFks.map((fk, idx) => {
    ...
    const selectFkParts = uniqueFks.map((_, idx) => `f${idx}.*`).join(", ");
    const genericSQL = `
    ...
    // Detect if this looks like a flow table
    ...
    if (colNames.includes("source_id") && colNames.includes("destination_id")) {
      ...
      `;
      console.log("🔗 Flow table detected. Joining node metadata...");
...
// Dataflow graph for a given node_id (source or destination)
app.get("/api/flow/:nodeId", async (req, res) => {
  ...
  try {
    const result = await pool.query(`
      SELECT 
        ...
        dst.node_type AS dest_node_type,

        -- Source name (from whichever table matches)
        COALESCE(ds_src.dataset_name, pn_src.processing_node_name, ur_src.role_name) AS source_name,

        -- Destination name (from whichever table matches)
        COALESCE(ds_dst.dataset_name, pn_dst.processing_node_name, ur_dst.role_name) AS destination_name

      FROM (
        SELECT t.*, f0.*
        FROM dataflow t
        LEFT JOIN poc f0 ON t.poc = f0.poc_id
      ) t
      ...
      -- Source name lookups
      LEFT JOIN dataset ds_src ON src.node_id = ds_src.node_id
      LEFT JOIN processing_node pn_src ON src.node_id = pn_src.node_id
      LEFT JOIN user_role ur_src ON src.node_id = ur_src.node_id

      -- Destination name lookups
      LEFT JOIN dataset ds_dst ON dst.node_id = ds_dst.node_id
      LEFT JOIN processing_node pn_dst ON dst.node_id = pn_dst.node_id
      LEFT JOIN user_role ur_dst ON dst.node_id = ur_dst.node_id

      WHERE t.source_id = $1 OR t.destination_id = $1
    `, [nodeId]);
    ...
    res.status(500).json({ error: "Failed to fetch flow for node" });
  }
});

app.post("/api/records/expand", async (req, res) => {
  const { table, column, value } = req.body;
  console.log(`Expanding records in table ${table} where ${column} = ${value}`);
